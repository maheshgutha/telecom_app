...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Map<String, dynamic>? _statsData;     // from /leads/stats
  Map<String, dynamic>? _adminData;     // from /reports/admin-analysis
  ...
  Future<void> _load() async {
    ...
    try {
      if (auth.user?.isAdmin == true) {
        // Admin: only needs admin-analysis (has overdueFollowupsCount built in)
        final adminResult = await api.getAdminAnalysis(auth);
        if (adminResult['error'] != null) throw Exception(adminResult['error']);
        _adminData = adminResult;
      } else {
        // Caller: only needs /leads/stats (has everything: calls, duration, wins, overdue, trends)
        final statsResult = await api.getLeadStats(auth);
        if (statsResult['error'] != null) throw Exception(statsResult['error']);
        _statsData = statsResult;
      }
    } catch (e) {
  ...
  // ── Getters from single source ─────────────────────────────────────────────
  // Caller stats (from /leads/stats)
  int get _myCallsCount => (_statsData?['todayCalls'] as Map?)?['count'] ?? 0;
  int get _myTalkSeconds => (_statsData?['todayCalls'] as Map?)?['duration'] ?? 0;
  ...
  List<int> get _trendThisWeek =>
      List<int>.from((_statsData?['trendThisWeek'] as List? ?? []).map((e) => (e as num).toInt()));
  List<int> get _trendLastWeek =>
      List<int>.from((_statsData?['trendLastWeek'] as List? ?? []).map((e) => (e as num).toInt()));

  // Admin stats (from /reports/admin-analysis)
  int get _teamCallsCount {
    final ts = _adminData?['teamStatus'] as List? ?? [];
    return ts.fold<int>(0, (s, c) => s + ((c['callsToday'] as int?) ?? 0));
  }
  int get _adminOverdue => _adminData?['overdueFollowupsCount'] ?? 0;
  int get _unassignedCount => _adminData?['unassignedCount'] ?? 0;
  List<Map<String, dynamic>> get _teamStatus =>
      List<Map<String, dynamic>>.from(_adminData?['teamStatus'] ?? []);
  List<Map<String, dynamic>> get _adminStatusCounts {
    // Build from teamStatus or use conversionFunnel
    final funnel = _adminData?['conversionFunnel'] as List? ?? [];
    if (funnel.isNotEmpty) return List<Map<String, dynamic>>.from(funnel.map((f) => {'_id': f['stage'], 'count': f['count']}));
    return [];
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(slivers: [
        ...
        else if (_error != null) SliverFillRemaining(child: _ErrorRetry(error: _error!, onRetry: _load))
        else SliverPadding(
          ...
          sliver: SliverList(delegate: SliverChildListDelegate([
            if (overdueCount > 0) ...[
              GestureDetector(
                ...
                child: _OverdueAlert(count: overdueCount),
              ),
            ...
            // Status chart
            if (!isAdmin && _statusCounts.isNotEmpty) ...[
              _StatusChart(statusCounts: _statusCounts), const SizedBox(height: 14),
            ],
            if (isAdmin && _adminStatusCounts.isNotEmpty) ...[
              _StatusChart(statusCounts: _adminStatusCounts), const SizedBox(height: 14),
            ],
            ...
            if (!isAdmin) ...[
              ...
              _WeeklyTrend(thisWeek: _trendThisWeek, lastWeek: _trendLastWeek),
              const SizedBox(height: 14),
            ],

            // Admin sections
            if (isAdmin && _adminData != null) ...[
              _AdminCallerPanel(teamStatus: _teamStatus, teamCalls: _teamCallsCount, fmt: _fmt),
              const SizedBox(height: 14),
              _AdminCampaignPanel(adminData: _adminData!),
              const SizedBox(height: 14),
  ...
  Widget _buildAdminKpis(BuildContext context) {
    return GridView.count(
      ...
      children: [
        _KpiTile('Team Calls Today', '$_teamCallsCount', Icons.phone_rounded, kPurple,
            sub: 'all callers',
            onTap: () => _showSheet(context, 'Team Calls Today', _Badge('$_teamCallsCount calls', kPurple),
                _teamStatus.isEmpty ? const Center(child: Text('No calls today')) :
                ListView.builder(itemCount: _teamStatus.length, itemBuilder: (_, i) {
                  final c = _teamStatus[i]; final u = c['user'] as Map? ?? {};
                  final calls = c['callsToday'] as int? ?? 0;
                  return ListTile(
                    leading: CircleAvatar(backgroundColor: kPurpleLight, child: Text((u['name'] as String? ?? 'U')[0].toUpperCase(), style: const TextStyle(color: kPurple, fontWeight: FontWeight.bold))),
                    title: Text(u['name'] ?? '', style: const TextStyle(fontWeight: FontWeight.w600)),
                    subtitle: Text(u['email'] ?? '', overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11)),
                    trailing: Text('$calls calls', style: const TextStyle(fontWeight: FontWeight.bold, color: kPurple)),
                    onTap: (u['_id'] as String?) != null ? () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (_) => CallerAnalysisScreen(userId: u['_id']!, userName: u['name'] ?? ''))); } : null,
                  );
                }))),
        ...
        _KpiTile('Unassigned', '$_unassignedCount', Icons.person_off_rounded, kAmber,
            sub: 'leads',
            onTap: _unassignedCount > 0 ? () => _showUnassignedSheet(context) : null),
        _KpiTile('Stale Leads', '${_adminData?['staleLeadsCount'] ?? 0}', Icons.hourglass_empty_rounded, Colors.grey.shade600,
            sub: '3+ days no call'),
      ],
    );
  }
    ...
    return GridView.count(
      ...
      children: [
        _KpiTile("Today's Calls", '$_myCallsCount', Icons.phone_rounded, kPurple,
            sub: 'by me',
            onTap: () => _showMyCallsSheet(context)),
        _KpiTile('Talk Time', _fmt(_myTalkSeconds), Icons.timer_rounded, kGreen,
            sub: 'today',
            onTap: _myTalkSeconds > 0 ? () => _showTalkTimeSheet(context) : null),
  ...
  void _showMyCallsSheet(BuildContext context) {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (_) => DraggableScrollableSheet(expand: false, initialChildSize: 0.75, maxChildSize: 0.95,
        builder: (_, ctrl) => _MyCallsSheet(api: api, auth: auth, ctrl: ctrl, callsCount: _myCallsCount, talkSecs: _myTalkSeconds, fmt: _fmt)));
  }
  ...
  void _showTalkTimeSheet(BuildContext context) {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(16))),
      builder: (_) => DraggableScrollableSheet(expand: false, initialChildSize: 0.75, maxChildSize: 0.95,
        builder: (_, ctrl) => _TalkTimeSheet(api: api, auth: auth, ctrl: ctrl, totalSecs: _myTalkSeconds, fmt: _fmt)));
  }
  ...
  void _showUnassignedSheet(BuildContext context) {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(16))),
      builder: (_) => DraggableScrollableSheet(expand: false, initialChildSize: 0.75, maxChildSize: 0.95,
        builder: (_, ctrl) => _UnassignedSheet(api: api, auth: auth, ctrl: ctrl, count: _unassignedCount)));
  }

  void _showSheet(BuildContext context, String title, Widget badge, Widget body) {
    showModalBottomSheet(context: context, isScrollControlled: true,
      shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(16))),
      builder: (_) => DraggableScrollableSheet(expand: false, initialChildSize: 0.7, maxChildSize: 0.95,
        builder: (_, ctrl) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
              child: Row(children: [Expanded(child: Text(title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16))), badge])),
...
class _MyCallsSheet extends StatefulWidget {
  ...
  final int callsCount, talkSecs; final String Function(int) fmt;
  const _MyCallsSheet({required this.api, required this.auth, required this.ctrl,
      required this.callsCount, required this.talkSecs, required this.fmt});
...
class _MyCallsSheetState extends State<_MyCallsSheet> {
  ...
  Widget build(BuildContext context) => Column(children: [
    ...
    Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), child: Row(children: [
      ...
      _Badge('${widget.callsCount} calls', kPurple),
      const SizedBox(width: 8),
      _Badge(widget.fmt(widget.talkSecs), kGreen),
    ])),
    const Divider(height: 1),
    Expanded(child: _loading ? const Center(child: CircularProgressIndicator())
      : _leads.isEmpty ? const Center(child: Text('No calls yet today', style: TextStyle(color: Colors.grey)))
...
class _TalkTimeSheetState extends State<_TalkTimeSheet> {
  ...
  Widget build(BuildContext context) => Column(children: [
    ...
    Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), child: Row(children: [
      ...
      _Badge('Total: ${widget.fmt(widget.totalSecs)}', kGreen),
    ])),
    const Divider(height: 1),
    Expanded(child: _loading ? const Center(child: CircularProgressIndicator())
      : _entries.isEmpty ? const Center(child: Text('No call logs for today', style: TextStyle(color: Colors.grey)))
...
class _UnassignedSheetState extends State<_UnassignedSheet> {
  ...
  @override void initState() { super.initState(); _load(); }
  Future<void> _load() async {
    final data = await widget.api.getLeads(widget.auth, filter: 'all', assignedTo: 'none', limit: 50);
    // Try different filter approaches
    final leadsData = data['leads'] as List? ?? [];
    final leads = leadsData.map((e) => Lead.fromJson(Map<String, dynamic>.from(e)))
        .where((l) => l.assignedToId == null || l.assignedToId!.isEmpty)
        .toList();
    if (mounted) setState(() { _leads = leads.isNotEmpty ? leads : leadsData.map((e) => Lead.fromJson(Map<String, dynamic>.from(e))).toList(); _loading = false; });
  }
  @override
  Widget build(BuildContext context) => Column(children: [
    _SheetHandle(),
    Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), child: Row(children: [
      const Expanded(child: Text('Unassigned Leads', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16))),
      _Badge('${widget.count} leads', kAmber),
    ])),
    const Divider(height: 1),
    Expanded(child: _loading ? const Center(child: CircularProgressIndicator())
      : _leads.isEmpty ? const Center(child: Text('No unassigned leads', style: TextStyle(color: Colors.grey)))
      : ListView.builder(controller: widget.ctrl, padding: const EdgeInsets.all(12), itemCount: _leads.length,
          itemBuilder: (_, i) {
            final l = _leads[i];
            return ListTile(
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12),
          border: Border.all(color: kBorder), boxShadow: [BoxShadow(color: color.withOpacity(0.08), blurRadius: 8)]),
...
class _UpcomingDemos extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.all(14),
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Row(children: [
        Icon(Icons.calendar_month_rounded, color: kGreen, size: 18), SizedBox(width: 8),
        ...
        return Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(10),
          ...
          child: Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (date != null) Text('📅 ${DateFormat('dd MMM, hh:mm a').format(date.toLocal())}', style: const TextStyle(color: kGreen, fontSize: 11, fontWeight: FontWeight.w600)),
            ])),
    ...
    return Container(padding: const EdgeInsets.all(14),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          _Dot(kPurple, 'This Week'), const SizedBox(width: 16), _Dot(const Color(0xFFCBD5E1), 'Last Week (tap ›)'),
        ]),
      ]),
    );
  }
...
class _AdminCallerPanel extends StatelessWidget {
  final List<Map<String, dynamic>> teamStatus;
  final int teamCalls; final String Function(int) fmt;
  ...
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.all(14),
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        _Badge('$teamCalls calls today', kPurple),
      ]),
      const SizedBox(height: 12),
...
class _AdminCampaignPanel extends StatelessWidget {
  ...
  const _AdminCampaignPanel({required this.adminData});
  @override
  Widget build(BuildContext context) {
    final camps = adminData['campaignPerformance'] as List? ?? [];
    if (camps.isEmpty) return const SizedBox();
    return Container(padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: kBorder)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Row(children: [
          Icon(Icons.campaign_rounded, color: kGreen, size: 18), SizedBox(width: 8),
          Text('Campaign Performance', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: kTextMain)),
        ]),
        const SizedBox(height: 12),
        ...camps.where((c) => c['_id'] != null).take(5).map((c) {
          final total = (c['totalLeads'] as int? ?? 0);
          final won = (c['won'] as int? ?? 0);
          // 'called' from admin-analysis = leads where totalCalls > 0
          ...
          return GestureDetector(
            ...
            child: Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(10),
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(children: [
                  Expanded(child: ClipRRect(borderRadius: BorderRadius.circular(4),
                      child: LinearProgressIndicator(value: calledPct.clamp(0.0, 1.0), minHeight: 5,
                          backgroundColor: Colors.grey.shade100, valueColor: const AlwaysStoppedAnimation<Color>(kPurple)))),
                  const SizedBox(width: 8),
                  Text('${(calledPct * 100).toInt()}%', style: const TextStyle(fontSize: 10, color: Colors.grey, fontWeight: FontWeight.bold)),
                ]),
              ]),
            ),
          );
